...
local function ProcessUnitOrders(unitID, frame)
    ...
    elseif uDef.isBuilder and not uDef.isFactory then
        ...
        if (not E.IsUnitBuildingFactory(unitID)) and NeedsOrders(unitID, false, false, false) then
            ...
            if frame % 30 == 0 and st.myFactoriesCount == 0 then
            ...
            if not tx and U.IsCommander(uDefID) and not E.IsUnitBuildingFactory(unitID) then
                -- Strict opening while we have no factory yet: the commander
                -- places exactly 1 mex and 4 wind/solar, one at a time (no
                ...
                if noFactoryYet then
                    if frame % 30 == 0 then
                        Spring.Echo(string.format("[OPEN] f=%d isCmd=%s opW=%d opM=%d mex=%d eco=%d", frame, tostring(U.IsCommander(uDefID)), st.openingWind or 0, st.openingMex or 0, st.mexUnitCount or 0, st.ecoEnergyCount or 0))
                    end
